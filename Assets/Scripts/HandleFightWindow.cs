...
public class HandleFightWindow : MonoBehaviour
{
    ...
    public Vector2 XYSpace = new Vector2();
    ...
    public void OnItemClicked(CubeItem clickedItem)
    {
        ...
        if (canDestroy)
        {
            ...
            SetScoreNum(destroyList.Count, true);
    ...
    private void GetSkillItemCount()
    {
        foreach (var item in itemArr)
        {
            if (item.IsSkillItem) { SkillItemCount++; }
        }
    }
    ...
    private bool FindDestroyItems(CubeItem rootItem)
    {
        ...
        if (destroyList.Count >= 3)
        {
            ...
            for (int i = 0; i < destroyList.Count; i++)
            {
                ...
                GameObject clean = Instantiate(Clean);
                clean.transform.SetParent(EffectRoot);
    ...
    private void DestroyBySkill(CubeItem item)
    {
        ...
        {
            case 5:
                if (IsLegal(x, y + 1)) { destroyList.Add(itemArr[x, y + 1]); }
                if (IsLegal(x, y - 1)) { destroyList.Add(itemArr[x, y - 1]); }

                if (IsLegal(x - 1, y)) { destroyList.Add(itemArr[x - 1, y]); }
                if (IsLegal(x - 1, y - 1)) { destroyList.Add(itemArr[x - 1, y - 1]); }
                if (IsLegal(x - 1, y + 1)) { destroyList.Add(itemArr[x - 1, y + 1]); }
                ...
                for (int i = 0; i < destroyList.Count; i++)
                {
                    ...
                    bomb.transform.SetParent(EffectRoot);
            ...
            case 6:
                ...
                GameObject lightingV = Instantiate(LightingVEffect);
                lightingV.transform.SetParent(EffectRoot);
                lightingV.GetComponent<RectTransform>().localPosition = new Vector2(x * XYSpace.x, 315);


                GameObject lightingH = Instantiate(LightingHEffect);
                lightingH.transform.SetParent(EffectRoot);
